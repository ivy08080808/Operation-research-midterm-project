...
def generate_moving_times(n_S, min_time=30, max_time=300):
    """
    ...
    for i in range(1, n_S + 1):
        for j in range(i + 1, n_S + 1):
            time = random.randrange(min_time, max_time + 1, 30)
            moving_times[(i, j)] = time
            moving_times[(j, i)] = time
...
def generate_instance(
    ...
    for order_id in range(1, n_K + 1):
        ...
        # Keep resampling until return time is within planning horizon.
        while True:
    ...
    # -------------------------
    moving_times = generate_moving_times(n_S)
...
if __name__ == "__main__":
    os.makedirs("generated_instances", exist_ok=True)
    ...
    for scenario_name, params in scenarios.items():
        for idx in range(1, 11):
            filename = f"generated_instances/{scenario_name}_{idx:02d}.txt"

            generate_instance(
                filename=filename,
                seed=1000 + idx + list(scenarios.keys()).index(scenario_name) * 100,
                **params
            )
